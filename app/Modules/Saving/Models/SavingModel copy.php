...
{
    protected $table         = PREFIX_TABLE.'sav_tabungan';
    ...
    public function getAll(array $whereAnd = [], array $whereOr = [], array $whereIn = [], array $orWhereIn = [], string $order = '', int $limit = 0, int $offset = 0)
    {
        ...
        $data = $this->db->table(PREFIX_TABLE.'sav_tabungan'.' f')
                    ->select('f.*, s.nama nama_santri, s.id_kelas, k.nama nama_kas, c.kelas')
                    ->join(PREFIX_TABLE.'_santri'.' s','s.id=f.id_santri','left')
                    ->join(PREFIX_TABLE.'sav_kas k','k.id=f.id_kas','left')
                    ->join(PREFIX_TABLE.'_kelas c','c.id=s.id_kelas','left')
                    ->having($whereAnd)
                    ->havingGroupStart()
                        ->orHaving($whereOr)
                    ->havingGroupEnd();
    ...
    public function getCount($start, $end)
    {
        ...
        return $this->db->table(PREFIX_TABLE.'sav_tabungan'.' f')
