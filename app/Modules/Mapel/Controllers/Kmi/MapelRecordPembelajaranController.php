...
class MapelRecordPembelajaranController extends BaseDataController
{
    ...
    public function input_data_from_detail($id_semester, $hari, $id_sesi, $tanggal)
    {
        ...
        $sesi = $this->sesiModel->getData($id_sesi);
        $sesi_before = (int)($sesi->sesi ?? 1) - 1;
        if ($sesi_before < 0)
            return FALSE;
        // var_dump($sesi, $sesi_before);
    ...
    public function get_data(string $id_semester, string $komplek, string $tanggal, string $id_sesi)
    {
        $this->model->db->close();   // tutup koneksi
        $this->model->db->initialize(); // buka koneksi baru

        return $this->model->addRelations([
            'id_kelas' => [
                'condition' => [
                    "komplek='$komplek'",
                ],
