        ...
        <el-form-item :class="['grow-0', gridItemClass(field?.colspan), formItemClass, formItemClass[field.nama_kolom], formItemClass['all']]"
          ...
          <div class="flex flex-wrap gap-x-3 w-full">
            ...
            <template v-if="field.input == 'date-wheel'">
              <date-wheel-picker
                :placeholder="!isEmpty(field.placeholder) ? field.placeholder : `Masukkan ${field.label}`" :class="['w-full flex-1',inputClass,eachInputClass?.[field.nama_kolom]]" 
...
export default {
  ...
  methods: {
    ...
    submitForm(){
      ...
      Object.values(this.fields).forEach(f => {
        if (form[f.nama_kolom])
          if (f?.function_submit) {
            form[f.nama_kolom] = this.runFunction({data:form[f.nama_kolom], func: f.function_submit})
          }
      })
      ...
      form.id = this.isEmpty(this.dataId) ? -1 : this.dataId
