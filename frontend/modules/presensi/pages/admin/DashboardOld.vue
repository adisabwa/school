<template>
  ...
  <div class="pt-4 px-0 md:px-3">
		<div class=" mb-4 lg:mb-6">
			<div class="text-4xl font-black text-slate-900 tracking-tight m-0">Academic Monitor</div>
			<div class="text-lg text-slate-500 font-medium">Pemantauan real-time kehadiran guru di setiap kelas.</div>
		</div>
		<!-- <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 mb-2
			...
			font-bold text-[var(--color-main-600)] ">
				{{ dateDayIndo(dateNow()) }} - Sesi {{ idSesi }}
			</div>
				...
				<div class="space-y-2 relative z-10">
					...
					<template v-if="scannedClasses[cls.id_kelas]">
						<div class="space-y-2 animate-in fade-in slide-in-from-bottom-2 duration-300">
							...
							<div class="bg-[var(--color-main-50)]/50 p-3 rounded-xl border border-[var(--color-main-100)]/30">
...
// 				this.idSesi = resSesi.data?.id || 2; // Default atau fallback
...
// 							'(id_sesi+{f}.jam)>': this.idSesi,
// 							'{n}id_sesi <=': this.idSesi,
// 							'{n}hari': this.dayIndo(dateNow),
// 						},
// 						order: ['kelas']
// 					}
// 				});
