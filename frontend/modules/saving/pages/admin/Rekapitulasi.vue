<template>
  <div id="rekapitulasi" class="py-6">
    <el-card>
      ...
      <el-row>
        <div :class="getDataFromStorage('vertical-menu') == '1' ? 'max-w-[calc(100vw-85px-var(--width-menu))]' : 'max-w-[calc(100vw-95px)]'">
          ...
          <div v-else class="w-full text-[14px]">
            <div class="mt-5">
              <div class="flex font-bold gap-3">
                <div class="bg-[var(--color-main-50)] py-2 px-3">Pengeluaran</div>
                <div class="bg-red-50 py-2 px-3">Pemasukan</div>
              </div>
            </div>
            <div class="my-5 overflow-x-scroll">
              <table-freeze v-if="rekapitulasi?.length > 0"
                ref="tableFreeze"
                :data="rekapitulasi">
                <template #header="{data}">
                  <tr class="bg-sky-900">
                    ...
                    <template v-for="col in data[0].column">
                ...
                <template #body="{data}">
                  <template v-for="(santri, ind) in data">
                    <tr>
                      <td nowrap valign="top" class="h-[35px] bg-white ">{{ ind + 1 }}</td>
                      <td nowrap valign="top" class="bg-white  text-left">{{ santri.text }}</td>
                      <td nowrap valign="top" class="bg-cyan-50 text-right font-bold">
                        {{ toIDR(santri.saldo) }}
                      </td>
                      <template v-for="(col, key) in santri.column">
                        <td class="bg-[var(--color-main-50)] text-right hover:bg-[var(--color-main-200)] pointer" valign="top" 
                          @click="getDetail(ind, key)" nowrap>
                          {{ col.pemasukan == 0 ? '-' : toIDR(col.pemasukan) }}
                        </td>
                        <td class="bg-red-50 text-right hover:bg-[var(--color-main-200)] pointer" valign="top" 
                          @click="getDetail(ind, key)" nowrap>
                          {{ col.pengeluaran == 0 ? '-' : toIDR(col.pengeluaran) }}
                        </td>
                      </template>
                      <td class="bg-[var(--color-main-100)] text-right"
                        valign="top" nowrap>
                  ...
                  <tr class="font-bold">
                    <td nowrap valign="top" class="h-[35px] bg-white"></td>
                    <td nowrap valign="top" class="bg-white text-left">Total</td>
                    <td nowrap valign="top" class="bg-cyan-50 text-right font-bold">
                      {{ toIDR(total.saldo) }}
                    </td>
                    <template v-for="(col, key) in total.column">
                      <td class=" bg-[var(--color-main-50)] text-right hover:bg-[var(--color-main-200)] pointer" valign="top"  nowrap>
                        {{ col.pemasukan == 0 ? '-' : toIDR(col.pemasukan) }}
                      </td>
                      <td class=" bg-red-50 text-right hover:bg-[var(--color-main-200)] pointer" valign="top" nowrap>
                        {{ col.pengeluaran == 0 ? '-' : toIDR(col.pengeluaran) }}
                      </td>
                    </template>
                      <td class=" bg-[var(--color-main-100)] text-right"
...
export default {
  ...
  setup(){
    ...
    return {
      isEmpty, toIDR, dateIndo, getDataFromStorage,
    }
  },
  components: {
  },
  ...
  methods: {
    getInitial: async function() {
      ...
      this.filter.bulan = isEmpty(filter.bulan) ? [dateNow(), dateNow()] : filter.bulan
