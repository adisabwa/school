...
export default {
  ...
  methods: {
    searchData(){
      ...
      this.params = {
        where: {
          ...
          'tanggal <= ' : setLastDateOfMonth(this.filter.end),
        },
        or: {},
        order:['tanggal desc'],
      }
    ...
    getInitial: async function() {
        this.loading = true;
        await this.$http.get('/kolom/preparation?table=' + this.$prefixTable + 'sav_tabungan&grouping=0&input=0')
          .then(result => {
            var res = result.data;
            this.fields = fillAndAddObjectValue(this.fields, res)
            this.loading = false
          });
  ...
  created: function() {
    ...
    this.filter.end = filter?.end ?? setLastDateOfMonth(dateNow())
    this.getInitial()
    console.log(setLastDateOfMonth(dateNow()));
  },
  mounted: function() {
    this.searchData()
  },
