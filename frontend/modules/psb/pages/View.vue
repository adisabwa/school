  ...
  export default {
    ...
    methods: {
      getParams(){
        ...
        if (!isEmpty(this.dataId) && this.dataId != -1)
      ...
      getInitial: async function() {
        this.loading = true
        await this.$http.get('/kolom/preparation?table=' + this.$prefixTable + 'psb&input=0&grouping=1')
          .then(result => {
            var res = result.data;
    ...
    mounted: function() {
      ...
      this.inputKeyword = isEmpty(filter.keyword) ? '' : filter.keyword
