...
export default {
  ...
  methods: {
    searchData(){
      let params = {
        where: this.filter.hari ? {
          hari:this.filter.hari,
        } : {},
        having:{},
      }
      if (this.filter.id_kelas)
        params.having.id_kelas = this.filter.id_kelas
      if (this.filter.id_guru)
        params.having.id_guru = this.filter.id_guru
      
      this.params = params
    },
    getInitial: async function() {
        this.loading = true;
        await this.$http.get('/kolom/preparation?table=' + this.$prefixTable + 'aka_penjadwalan&grouping=0&input=0')
          .then(result => {
            var res = result.data;
            this.fields = {...{
              kelas:{
  ...
  created: function() {
    ...
    fillObjectValue(this.filter, filter)
    this.getInitial()
