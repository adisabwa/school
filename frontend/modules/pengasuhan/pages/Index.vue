      ...
      <el-card class="bg-white/[0.7]"
        ...
        <div :class="[scrollY > showHidden ? 'opacity-0' : 'opacity-100',
          ...
          <div class="flex items-center justify-between">
            <div width="100px" class="text-center flex items-center justify-start gap-3 font-bold">
              <icons v-if="(currentPage - perPage) >= 0" icon="mdi:arrow-left" class="text-xl m-0 cursor-pointer" @click="currentPage = currentPage - perPage"/>
              <span class="text-md">Nilai ke ({{ currentPage + 1 }} - {{ endPage > totalPage ? totalPage : endPage }}) / {{ totalPage }}</span>
              <icons v-if="(currentPage + perPage) <= totalPage" icon="mdi:arrow-right" class="text-xl m-0 cursor-pointer" @click="currentPage = currentPage + perPage"/>
            </div>
          ...
          <table-freeze
            ...
            <template #header="{data}">
              <tr class="*:border *:border-solid *:border-slate-300">
                ...
                <template v-for="(ujian, key) in data[0].nilai.slice(currentPage, currentPage + perPage)" :key="key">
            ...
            <template #body="{data}">
              <tr v-for="(d, key) in data">
                <td>{{ key + 1 }}</td>
                <td>{{ d.nama }}</td>
                <td v-for="(ujian) in d.nilai.slice(currentPage, currentPage + perPage)" :key="ujian.id" class="text-center">
    ...
    methods: {
      ...
      getData(){
        ...
        this.$http.get('pengasuhan/nilai',{
          ...
          this.totalPage = this.dataNilai?.[0]?.nilai?.length || 0
          setTimeout(() => {
            this.$refs.tableFreeze?.getFreezeHeader()
            this.loading = false;
          }, 300)
