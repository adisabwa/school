  ...
  <div class="relative bg-white/70 sm:overflow-hidden w-screen h-screen flex flex-col">
    ...
    <div class="bg-slate-700 flex justify-end h-12 p-2 gap-x-5 items-center">
      ...
      <div class="shadow-md flex max-sm:absolute left-4 top-[47%] max-sm:-translate-y-1/2 z-[100] flex-col sm:flex-row">
        ...
        <div class="bg-white text-teal-950
          w-[80px] sm:w-[60px]
          ...
          <span class="font-sans text-center"> 
            ...
            <el-input v-model="currentMatch.partai" type="text" class="[&_*]:text-teal-950 text-[32px] [&_*]:font-bold w-full [&_*]:text-center
              focus:[outline:none] border-0 [&_*]:shadow-none border-b-2 border-solid border-teal-950" />
          </span>
      ...
      <div class="shadow-md flex gap-x-2 gap-y-1 max-sm:absolute right-4 top-[47%] max-sm:-translate-y-1/2 z-[100] flex-col sm:flex-row">
        <div v-for="b in range(3, 1)" 
          :class="[b != currentMatch.babak ? 'bg-teal-900/50 text-white' : 'bg-white text-teal-900',
            'flex-none px-3 border border-solid border-teal-700 rounded-md py-1',
      ...
      <div class="*:leading-none">
        ...
        <el-button v-else class="px-5 py-3 h-fit
          ...
          bg-teal-800 pointer hover:scale-90"
          @click="finishMatch">
          Finish Match
        </el-button>
      </div>
          ...
          <div :class="['md:w-1/2 h-1/2 flex flex-col gap-y-3 px-4 py-3',
            ...
            <div class="grid grid-cols-2 font-bold text-center gap-2">
              <div v-for="no in range(4, 1)">
                ...
                this.isEmpty(currentMatch[`key_juri_${no}`]) ? 'bg-slate-700 !text-slate-400' :
                ...
                <div :class="['text-[12px] mt-1 leading-none cursor-pointer hover:scale-80',
                  this.isEmpty(currentMatch[`key_juri_${no}`]) ? '!text-slate-300 ' : '!text-white']"
                  @click="resetKey(no)">
                  {{ this.isEmpty(currentMatch[`key_juri_${no}`]) ? 'Juri Belum Masuk' : 'Juri sudah masuk'}}
                </div>
              </div>
            </div>
          </div>
