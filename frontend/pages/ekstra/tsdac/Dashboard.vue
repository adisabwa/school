  ...
  methods:{
    ...
    setMatch(){
      ...
      this.$http.post('ekstra/ts/tsdac/penilaian/get_current_match')
        .then(res => {
          let data = res.data
          let key = this.getDataFormStorage('key_match')
          if (!this.isEmpty(data.key_match)) {
    ...
    setJuri(no){
      ...
      this.$http.post('ekstra/ts/tsdac/penilaian/get_current_match')
        .then(res => {
          let data = res.data
          let key = this.getDataFormStorage('key_juri')
          let exist = -1
          for(let no_juri in this.range(4,1 )) {
            if (data[`key_juri_${no_juri}`])
              exist = no_juri
          }
          // console.log(key_juri, data[key_juri], this.isEmpty(data[key_juri]))
          if (!this.isEmpty(data[key_juri])) {
