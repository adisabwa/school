<template>
  <div class="space-y-5 bg-white/[0.8] p-2 pb-6 pt-10 md:pt-2">
    ...
    <el-dialog v-model="showSummaryDetail"
      ...
      <template #header>
        <div class="bg-slate-900 p-5 lg:p-7 pb-3 lg:pb-4 text-white relative">
          <div class="flex items-center gap-2 text-emerald-400 font-black text-md uppercase tracking-widest mb-2 lg:mb-4">
      ...
      <div class="bg-white w-full max-w-5xl relative z-10 overflow-hidden flex flex-col max-h-[calc(100vh-200px)]">
        <el-tabs type="border-card" class="[&_.is-active]:text-emerald-700">
          <el-tab-pane>
            <template #label>
              ...
              <div class="space-y-2">
                ...
                <table class="w-full text-left">
                  <thead class="font-semibold uppercase">
                    ...
                    <tr class="bg-slate-100 text-slate-500">
                      <td class="px-2 py-2 text-center w-[60px] text-emerald-600">Hadir</td>
                  ...
                  <tbody class="divide-y divide-slate-50">
                    <tr v-for="(s, idx) in summaryStudents" :key="s.id" class="hover:bg-slate-50/30 transition-colors group">
                      ...
                      <td class="px-2 py-2 text-center font-semibold text-lg text-emerald-700">{{ s.total_hadir }}</td>
                      ...
                      <td class="px-4 py-2 text-right font-bold text-emerald-700">
                        {{ s.presentase_hadir }} %
                        <div :style="{width: s.presentase_hadir + '%'}" class="h-[4px] rounded-full bg-emerald-700"></div>
                      </td>
                    </tr>
                  </tbody>
                </table>
      ...
      <template #header>
        <div class="bg-slate-900 p-5 lg:p-7 pb-3 lg:pb-4 text-white relative">
          ...
          <div class="flex items-center gap-1 bg-white/5 p-2 rounded-md border border-white/10 w-full">
            <icons icon="mynaui:book-open" class="text-emerald-400" />
                  ...
                  <td class="px-2 py-2">
                    <div class="flex justify-center">
                      <span :class="[`px-2 py-1 rounded-xl text-[11px] font-black uppercase tracking-widest ${s.kehadiran == 'alfa' ? 'bg-red-100 text-red-600' : 
                        s.kehadiran == 'hadir' ? 'bg-emerald-100 text-emerald-600' : 'bg-orange-100 text-orange-600'}`]">
                        {{ s.kehadiran }}
                      </span>
                    </div>
                  </td>
    ...
    findDateStart(){
      switch (this.activePeriod) {
        case 'HARIAN':
          ...
          break
        case 'MINGGUAN':
          this.dateFrom = this.getStartAndEndOfWeek(this.dateNow()).startOfWeek
          break
        case 'BULANAN':
          this.dateFrom = this.getStartAndEndOfMonth(this.dateNow()).startOfMonth
          break
        case 'SEMESTER':
    ...
    findDateEnd(){
      switch (this.activePeriod) {
        case 'HARIAN':
          ...
          break
        case 'MINGGUAN':
          this.dateEnd = this.getStartAndEndOfWeek(this.dateFrom).endOfWeek
          break
        case 'BULANAN':
          this.dateEnd = this.getStartAndEndOfMonth(this.dateFrom).endOfMonth
          break
        case 'SEMESTER':
          let sem = this.optionsSemester.filter(d => d.value == this.idSemester)?.[0] ?? {}
