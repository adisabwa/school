          ...
          <table-freeze v-else
            ...
            <template #header="{data}">
              <tr class="*:border *:border-solid *:border-slate-300">
                ...
                <th v-for="(mapel, key) in [...data[0].mapel,...['Jumlah','Rata-Rata','Ranking']]"
                  :key="key"
                  class="">
                  <div
            ...
            <template #body="{data}">
              <tr v-for="(d, key) in data"
                ...
                <td>{{ key + 1 }}</td>
                <td>{{ d.nama }}</td>
                <td v-for="(mapel, mKey) in d.mapel"
                  :key="mKey"
                  class="text-center ">
                  {{ mapel.uts }}
                </td>
                <td class="text-center font-bold">{{ d.total_uts }}</td>
                <td class="text-center font-bold">{{ (d.total_uts / d.mapel.length).toFixed(2) }}</td>
