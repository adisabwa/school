              ...
              <tr v-for="(d, key) in data"
                ...
                <template v-for="(mapel, mKey) in d.pengasuhan">
                  <td :class="['text-center px-2',
                    ...
                    {{ mapel.type == 'number' ? mapel.nilai : setLabelNilai(mapel.nilai)}}
                  </td>
                </template>
              </tr>
            </template>
...
<script setup>
  import { setLabelNilai } from '@/helpers/pengasuhan';
</script>
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async fetchAndDownloadRaport(id) {
        try {
          const result = await this.$http.get('rapor/download_raport_pengasuhan', {
            params: {
              id_semester: this.filter.id_semester,
              id_kelas: this.filter.id_kelas,
              id_santri: id,
